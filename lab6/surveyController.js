...
module.exports = function(app){
    ...
    app.get('/analysis', function(req, res){
        var emailaddress = readData("emailaddress");
        ...
        var part = readData("part");
        var zzz=readData("zzz");
        res.render('showResults', {results: [emailaddress,favorite,firstname,gen,gender,guilty,lastname,part,zzz]});
        console.log([emailaddress,favorite,firstname,gen,gender,guilty,lastname,part,zzz]);
    });
    ...
    app.post('/niceSurvey', urlencodedParser, function(req, res){
        ...
        for (var key in json){
            ...
            if ((key === "part") && (json[key].length === 2)){
                for (var item in json[key]){
                    combineCounts(key, json[key][item]);
                }
            }
            else if ((key === "part") && (json[key].length === 3)){
                for (var item in json[key]){
                    combineCounts(key, json[key][item]);
                }
            }
